...
public class NetworkManagerTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (transformedName.replace(".", "/").equals("net/minecraft/network/NetworkManager")) {
            ...
            insns.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "net/raphimc/openauthmod/OpenAuthMod", "getInstance", "()Lnet/raphimc/openauthmod/OpenAuthMod;", false));
            insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
            insns.add(new VarInsnNode(Opcodes.ALOAD, 2));
            insns.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "net/raphimc/openauthmod/OpenAuthMod", "handlePacket", "(Lnet/minecraft/network/NetworkManager;Lnet/minecraft/network/Packet;)Z", false));
